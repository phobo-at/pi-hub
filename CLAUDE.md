...
- `smart_display/watch_faces.py` — stdlib-only helpers for the three hero-clock variants (`classic` digital, `qlocktwo` German word-clock grid, `analog` SVG). Exposes `qlocktwo_active_cells()` and `analog_hand_angles()` for server-side initial render; the same logic is mirrored in `app.js` for subsequent ticks. If you change the QLOCKTWO grid or word coordinates, update **both** sides or tests will drift from the UI.
