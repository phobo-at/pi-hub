...
- `smart_display/app.py` — `create_app()` wires config, the `StateStore`, the `ImageCache`, all providers, and the `Scheduler`. Each provider is registered as a `ScheduledJob` with its own refresh interval. `serve_app()` runs Waitress.
- `smart_display/scheduler.py` — one daemon thread per job, runs `task()` immediately then on a fixed interval. Exceptions are logged, never raised — a failing provider must not take down the loop.
- `smart_display/state_store.py` — the **only** writable state. Thread-safe (single `Lock`). Every mutation calls `_persist_locked()` which writes `data/last_good.json` via `DiskCache`. On startup, the store rehydrates from `last_good.json` so the UI never boots empty after a power cycle.
...
- Status badges are hidden in the normal `ok` state by design — don't reintroduce them. Same for the removed "Startbildschirm" heading and the standalone weather card on the right. See `AGENTS.md` "Aktueller UI-Maßstab".
